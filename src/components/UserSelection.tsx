...
const UserSelection: React.FC<UserSelectionProps> = ({ onGoToSignUp, onGoToForgotPassword, language }) => {
  ...
  const text = {
    ...
    emailPlaceholder: { ko: "이메일 입력", en: "Enter email" },
    ...
    errorInvalid: { ko: "이메일 또는 비밀번호가 잘못되었습니다.", en: "Invalid email or password." },
    errorRequired: { ko: "이메일과 비밀번호를 입력해주세요.", en: "Please enter your email and password." },
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.code === 'auth/invalid-credential' || error.code === 'auth/user-not-found' || error.code === 'auth/wrong-password') {
          setError(text.errorInvalid[language]);
      } else {
      ...
      <div className="flex min-h-screen w-full flex-col items-center justify-center p-4">
        <div className="w-full max-w-md space-y-6">
          ...
          <form className="space-y-4" onSubmit={handleLogin}>
            ...
            <button className="w-full h-14 flex items-center justify-center rounded-lg bg-primary text-white text-base font-bold transition-colors hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 dark:focus:ring-offset-background-dark disabled:bg-slate-400" type="submit" disabled={loading}>
              {loading ? <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-white"></div> : text.login[language]}
            </button>
          </form>
